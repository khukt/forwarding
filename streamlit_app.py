...
class World:
    ...
    sessions: Dict[str, Dict[str, object]] = field(default_factory=dict)  # key: client proxy id → {user, 2fa_passed}
...
def is_authed(w: World, client_proxy: str) -> bool:
    return client_proxy in w.sessions and "user" in w.sessions[client_proxy]
...
if 'world_bank_rf' not in st.session_state:
    st.session_state.world_bank_rf = init_banking_world()
...
with _tabs[0]:
    ...
    with right:
        ...
        _g_val = st.slider("Step", 1, len(GUIDE_STEPS), value=st.session_state.guide_step_idx_rf, key="guide_step_ui_rf")
        if not st.session_state.guide_is_playing_rf:
            st.session_state.guide_step_idx_rf = _g_val

        spd = st.select_slider("Animation speed", options=["slow", "normal", "fast"], value="normal", key="anim_speed_bank_guide_rf")
    ...
    if st.session_state.guide_step_idx_rf == 1:
        ...
        hi_edges = [("P1", "P3"), ("P2", "P3")]
    elif st.session_state.guide_step_idx_rf == 6:
...
with _tabs[1]:
    ...
    with st.sidebar:
        ...
        if st.button("Invoke", key="play_invoke_rf"):
            ok, route, edges = invoke_and_shortcut(pw, client) if do_short else invoke_route(pw, client)[:3]
...
with _tabs[2]:
    ...
    st.caption("Demonstrates login, 2FA, and account ops traversing proxy → skeleton → object.")
    ...
    with run_cols[0]:
        if st.button("Run Flow", key="flow_run_rf"):
            ...
            if not ok:
                st.warning("Routing failed (no skeleton). Fix topology and try again.")
            else:
    ...
    with run_cols[1]:
        ...
        draw_world(fw, highlight_edges=edges2, title=f"Flow Route: {route2}")
