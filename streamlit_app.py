...
# (2025-09-25) — fixes: summer boost param, clearer risk thresholds, hotspot counter, bold heat, map ordering
...
def sensor_model_step(sid, idx_t, env, p, summer_boost_C):
    ...
    # ---- FIX: use PASSED parameter (not outer var) & blend so effect is visible even with low sun ----
    if p.get("hotspot", False):
        rail += float(summer_boost_C) * (0.45 + 0.55 * (sun**2.2))
    ...
    return dict(light_lux=int(round(light)), irradiance_Wm2=round(irr,1),
                ...
                risk_score=round(risk,3), exceeded=",".join(flags), hotspot=bool(p.get("hotspot", False)))
...
with tab_map:
    ...
    # Build static layers
    tile_layer, static_bs_rings, static_path_coords, hot_layer = static_layers(
        SECS, st.session_state.route_df, use_tiles, show_hotspot_rings
    )

    # ------ Compute frame physics/comm & sensors ------
    t=st.session_state.t_idx
    def build_frame_for_t(t_idx):
        ...
        return dict(t=t_idx,trainA=trainA,segment=seg,quality=quality,cap_bps=cap_bps,rand_loss=rand_loss,sensors=sensors)

    if "_frame" not in st.session_state:
        st.session_state._frame=build_frame_for_t(0)
    ...
    with colL:
        ...
        # ---- Geographic tagging: mark sensors inside HOTSPOTS as hotspot=True (runtime) ----
        if demo_force_issues and len(sensors_base):
            ...
            # ---- NEW: quick badge so you can confirm it's working
            hot_count = int(in_any.sum())
            st.caption(f"Hotspot sensors this tick: **{hot_count} / {len(sensors_base)}**")
        ...
        per_single=per_from_snr(snr_table.get(bearer,-20.0))
        ...
        def sensor_row(r):
            ...
            # ---- FIX: slightly more sensitive so injection shows clearly
        ...
        laneA_bps=len(laneA_alerts)*BYTES_ALERT*(2 if secondary else 1)
        ...
        """
        with c_topM: st.markdown(chip_html, unsafe_allow_html=True)

        # -------------------- Dynamic map layers (heat first) --------------------
        def dynamic_layers(tsr_list,train_pos,sensors_df,quality_macro):
            # Bold per-segment heat colored by nearest sensor risk
            seg_rows=[]
            if isinstance(sensors_df, pd.DataFrame) and not sensors_df.empty and "label" in sensors_df.columns:
                latv=sensors_df["lat"].to_numpy(); lonv=sensors_df["lon"].to_numpy()
                path_np=np.array(static_path_coords)
                d2=(path_np[:,1][:,None]-latv[None,:])**2+(path_np[:,0][:,None]-lonv[None,:])**2
                nearest_idx=np.argmin(d2,axis=1); labels=sensors_df["label"].to_numpy()
                col_map={"low":CBL["green"]+[255],"medium":CBL["orange"]+[255],"high":CBL["red"]+[255]}
                for i in range(len(static_path_coords)-1):
                    lbl=str(labels[nearest_idx[i]]); seg_rows.append({"path":[static_path_coords[i],static_path_coords[i+1]],
            ...
            # Order: heat on top of tiles/rings, but under points
            return [heat_layer,sens_layer,text_layer,tsr_layer,halo_layer,icon_layer]

        def deck_map(tsr_list,train_pos,sensors_df,quality_macro):
        ...
        with colRW:
            ...
            st.pydeck_chart(deck_map(st.session_state.tsr_polys_real,trainA,sensors,quality),
                            use_container_width=True,height=520)
        with colTMS:
            st.markdown('<span class="ribbon">TMS VIEW</span>', unsafe_allow_html=True)
            st.pydeck_chart(deck_map(st.session_state.tsr_polys_tms,trainA,sensors,quality),
                            use_container_width=True,height=520)
    ...
    with colR:
        ...
        # Recompute basic numbers at current t from saved frame (quick)
        f=st.session_state._frame
        _,_,quality=nearest_bs_quality(*f["trainA"])
        cap_bps_train,_=cap_loss(quality, f["t"])
        # To avoid recomputing everything again, approximate chart inputs from recent alerts/messages
        raw_bps = int((sensors["raw_bps"]).sum()) if "raw_bps" in sensors.columns else 0
        laneA_bps = 280*len(st.session_state.alerts_feed[-6:])   # rough view
        laneB_bps = 180 if "maintenance_summary" else 0
        snr_use = 0.0
        laneA_success = 1.0
        lat_ms = 40.0

        t_cur=f["t"]
        if math.isnan(st.session_state._times[t_cur]):
            st.session_state._times[t_cur]=t_cur
            st.session_state.arr["raw_bits"][t_cur]=raw_bps
            st.session_state.arr["laneA_bits"][t_cur]=laneA_bps
            st.session_state.arr["laneB_bits"][t_cur]=laneB_bps
            st.session_state.arr["cap_bits"][t_cur]=cap_bps_train
            st.session_state.arr["snr_db"][t_cur]=snr_use
            st.session_state.arr["laneA_succ"][t_cur]=laneA_success
            st.session_state.arr["lat_ms"][t_cur]=lat_ms
            st.session_state.qual[t_cur]=quality; st.session_state.seg[t_cur]=f["segment"]
...
with tab_flow:
    ...
    if "arr" not in st.session_state: st.info("Press ▶ Play to populate telemetry.")
    else:
        # Use the arrays captured in Map tab
        arr=st.session_state.arr; t=st.session_state.t_idx
